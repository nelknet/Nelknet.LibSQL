...
public class SpecialFeaturesTests : IDisposable
{
    ...
    {
        // Test that :memory:?cache=shared does not provide actual shared cache behavior
        // The libSQL experimental API doesn't support URI parameters, so shared cache doesn't work
